...
function fromDeadlineSubmitPayload({ id, is_finished, finish_at, file_deadlineSubmit_lesson, deadline }: DeadlineSubmitPayload): Deadline {
  return {
    ...
    lessonId: deadline.lesson.id,
    courseId: deadline.lesson.course.id,
    lessonName: deadline.lesson.name,
    courseName: deadline.lesson.course.name,
    courseCode: deadline.lesson.course.mskh,
    locationItems: [],
